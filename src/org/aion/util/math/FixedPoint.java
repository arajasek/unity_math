...
public class FixedPoint {
    ...
    public FixedPoint(BigInteger value) {
        ...
        if (value.signum() < 0) {
            throw new RuntimeException("Cannot create a FixedPoint with negative value");
        }
    ...
    public FixedPoint(BigDecimal value) {
        if (value.signum() < 0) {
            throw new RuntimeException("FixedPoint can only represent positive rational numbers");
        } else {
            try {
                this.value = value.multiply(new BigDecimal(MAX_PRECISION)).toBigInteger();
            } catch (ArithmeticException arithmeticException) {
                throw new RuntimeException("Precision would be lost when creating this FixedPoint. " +
    ...
    public FixedPoint divideByPowerOfTwo(int shift) {
        ...
        if (shift <= 0) {
            throw new RuntimeException("Can only divide by a positive power of two");
        }
